<!-- 
 翻译来源：https://github.com/shanraisshan/claude-code-best-practice/blob/main/best-practice/claude-settings.md
 翻译时间：2026-06-17 02:09:32 CST
 翻译版本：v2.1.178
-->
...
Claude Code `settings.json` 文件中所有可用配置选项的综合指南 in Claude Code's `settings.json` files. 截至 v2.1.178，Claude Code 提供 **80+ 设置**和 **200+ 环境变量** （使用 `settings.json` 中的 `"env"` 字段以避免包装脚本）.
...
**托管设置** are organization-enforced and cannot be overridden by any other level, including command line arguments. Delivery methods:
...
- 托管设置 may lock or override local behavior even if local files specify different values.
...
| `fastModePer会话OptIn` | boolean | `false` | Require users to opt in to fast mode each session |
...
|--------|---------|---------|
| `//` | Absolute path from filesystem root | `Read(//用户s/alice/file)` |
...
> **Reserved server name (v2.1.128):** `workspace` is a reserved MCP server name. 用户-defined servers with this name are skipped at load time with a warning logged to the session log. Rename any pre-existing `workspace` server to avoid the collision.
...
|-----|------|-------|-------------|
| `enableAll项目McpServers` | boolean | Any | Auto-approve all `.mcp.json` servers |
...
| `extraKnownMarketplaces` | object | 项目 | Add custom plugin marketplaces (team sharing via `.claude/settings.json`) |
...
| `CLAUDE_CODE_SKIP_PROMPT_HISTORY` | Set to `1` to skip writing prompt history and session transcripts to disk. 会话s started with this variable set do not appear in `--resume`, `--continue`, or up-arrow history. Useful for ephemeral scripted sessions |
...
| `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS` | 会话End hook timeout in ms (replaces hard 1.5s limit) |
