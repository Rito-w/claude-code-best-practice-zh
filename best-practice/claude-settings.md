...
Claude Code `settings.json` 文件中所有可用配置选项的综合指南。截至 v2.1.167,Claude Code 提供 **80+ 设置项**和 **200+ 环境变量**(使用 `settings.json` 中的 `"env"` 字段可避免使用包装脚本)。
...
|-----|------|---------|-------------|
| `parentSettingsBehavior` | string | `"first-wins"` | 控制 SDK 父进程(嵌入宿主进程)提供的 managed 设置在已存在管理员部署的 managed 层级时是否生效。`"first-wins"`:丢弃父级提供的设置,仅应用管理员层级。`"merge"`:父级设置在管理员层级下生效,但经过过滤,使其只能**收紧**策略而不能放宽。需要 v2.1.133+ |
| `policyHelper` | object | - | 管理员部署的可执行文件,在启动时动态计算 managed 设置。对象形状:`{path: string}` 指向 helper 二进制文件。仅在 MDM 或系统级 `managed-settings.json` 文件中生效(不会从用户/项目设置中读取)。Helper 输出在每次启动时合并到 managed 层级。需要 v2.1.136+ |
| `requiredMinimumVersion` | string | - | **(仅 Managed)** 如果安装版本低于此最低版本,阻止 Claude Code 启动。CLI 退出并提示用户升级。与 `minimumVersion`(控制自动更新最低版本)互补--此项在启动时强制执行。示例:`"2.1.163"` |
| `requiredMaximumVersion` | string | - | **(仅 Managed)** 如果安装版本超过此最高版本,阻止 Claude Code 启动。版本过新时 CLI 退出报错。与 `requiredMinimumVersion` 配合使用可在托管环境中锁定特定版本范围。示例:`"2.1.165"` |
...
| `"bypassPermissions"` | 跳过所有权限检查(危险)。所有基于路径的提示均已跳过--写入 `.git`、`.config/git`、`.claude`、`.vscode`、`.idea`、`.husky`、`.cargo`、`.devcontainer`、`.yarn` 和 `.mvn` 不再提示(**v2.1.121** 豁免了 `.claude/commands/`、`.claude/agents/`、`.claude/skills/` 和 `.claude/worktrees/`;**v2.1.126** 移除了所有剩余的基于路径的提示)。只有针对文件系统根目录或 home 目录的删除操作(`rm -rf /`、`rm -rf ~`)仍会提示,作为防止模型错误的断路器 |
